...
async function initCuisine() {
  ...
  allRestaurants = await PlacesAPI.fetchRestaurantsForRegion(
    initialFetchRegion,
    18,
  );
...
function setupSearchAndFilters() {
  ...
  if (regionsSelect)
    regionsSelect.addEventListener('change', () => {
      ...
      try {
        location.href = addRegionToUrlString(location.href, region);
      } catch (e) {
        loadRegion(region);
      }
    });
...
function applyFilters() {
  ...
  if (region && region !== 'All' && region !== initialFetchRegion)
  ...
  list.sort(
    (a, b) =>
      (fav.restaurants.includes(a.placeId) ? 0 : 1) -
      (fav.restaurants.includes(b.placeId) ? 0 : 1),
  );
...
function renderRestaurants() {
  ...
  if (!currentResults || currentResults.length === 0)
    ...
    const active = sel ? sel.value : 'All';
    if (active && active !== 'All') mapHelpers.setView(active);
