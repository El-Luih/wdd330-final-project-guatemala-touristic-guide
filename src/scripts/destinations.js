...
async function init() {
	...
	if (regionsSelect) {
		...
		regionsSelect.addEventListener('change', () => selectRegion(regionsSelect.value));
	}
...
function selectRegion(region) {
	...
	try {
		...
		document.querySelectorAll('.filter-button').forEach(b => b.classList.toggle('active', b.dataset.region === region));
		loadRegion(region);
	}
}
...
function applyFilters(region = 'All', status = 'all') {
	...
	if (status && status !== 'all') {
		...
		if (status === 'closed') list = list.filter(p => (typeof p.isOpen === 'boolean') ? !p.isOpen : (p.status || '').toLowerCase().includes('closed'));
	}
	...
	if (currentResults.length > PAGE_SIZE) {
		...
		if (!btn) {
			...
			btn.addEventListener('click', () => {
				...
				const e = Math.min(currentResults.length, (currentPage + 1) * PAGE_SIZE);
