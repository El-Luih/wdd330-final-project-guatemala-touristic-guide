...
function createEventCard(evt) {
  ...
  img.src = imageFile
    ? `${base}images/events/${imageFile}`
    : `${base}images/placeholder-2x1.svg`;
...
async function loadAndRenderEvents() {
  ...
  for (const p of candidates) {
    try {
      ...
      try {
        console.log('events.json body', data);
      } catch (e) {
        /* Ignore error */
      }
      break;
  ...
  data.forEach((raw) => {
    ...
    const displayDate = special
      ? special
      : `${String(startMD.mm).padStart(2, '0')}-${String(startMD.dd).padStart(2, '0')}`;
