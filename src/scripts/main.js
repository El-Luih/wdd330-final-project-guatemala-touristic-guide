...
async function showWeatherFor(lat, lon, regionName = 'Guatemala') {
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      img.height = 48;
      img.onerror = () => {
        img.src = `${baseImg}images/weather/clear.svg`;
      };
...
function setupMainRegionButtons() {
  ...
  filterContainer.addEventListener('click', (e) => {
    ...
    try {
      ...
      document
        .querySelectorAll('.filtering-buttons button')
        .forEach((b) =>
...
function markCurrentRegion(region) {
  try {
    document
      .querySelectorAll('.filtering-buttons button')
      .forEach((b) =>
        b.classList.toggle('current', b.dataset.region === (region || 'All')),
      );
  } catch (e) {
    /* Ignore error */
  }
    ...
    try {
      ...
      if (restContainer) {
        ...
        (rs || []).slice(0, 6).forEach((p) => {
          ...
          favBtn.type = 'button';
          favBtn.className = 'fav-button';
          favBtn.textContent = isFavorite(r.id, 'restaurant') ? '★' : '☆';
          favBtn.addEventListener('click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            toggleFavorite(r.id, 'restaurant');
            favBtn.textContent = isFavorite(r.id, 'restaurant') ? '★' : '☆';
          });
          card.appendChild(favBtn);
...
async function renderMainEventsGadget() {
  ...
  try {
    console.debug('Events JSON fetch candidates:', candidates);
  } catch (e) {
    /* Ignore error */
  }
  let data = null;
  ...
  for (const p of candidates) {
    try {
      ...
      try {
        console.log('main events body', data);
      } catch (e) {
        /* Ignore error */
      }
      break;
  ...
  if (!data) {
    ...
    attempts.forEach((a) => {
      ...
      if (a.error) li.textContent = `${a.path} → error: ${a.error}`;
        ...
        li.textContent = `${a.path} → ok: ${a.ok} ${a.status ? `(HTTP ${a.status})` : ''}`;
      list.appendChild(li);
    });
  ...
  events.forEach((e) => {
    ...
    date.textContent = sd.toLocaleDateString(undefined, {
      month: 'short',
      day: 'numeric',
    });
